...
public class Inbox extends javax.swing.JPanel {
    ...
    public Inbox(WorkQueue workQueue)
    {
        ...
        System.out.println("Patient inbox: " + workQueue.getWorkRequestList());
        populateInbox();
    }
    ...
    private void btnOpenMailActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOpenMailActionPerformed
     ...
     JFrame frame = new JFrame("Mail from " + mail.getSender().getUsername());
        ...
        if(selectedRow > -1)
        {
            frame.add(new ViewMail((Mail) tblInbox.getValueAt(selectedRow, 0)));
        }
         } else {
    ...
    private void populateInbox()
    {
        ...
        dtm.setRowCount(0);
        PatientMail patientMail = null;
        
        for(WorkRequest workRequest: workQueue.getWorkRequestList())
        {
            if(workRequest.getClass() == PatientMail.class)
            {
                patientMail = (PatientMail) workRequest;
                Object[] row = new Object[4];
                row[0] = patientMail;
                row[1] = patientMail.getRequestDate();
                row[2] = patientMail.getSubject();
                row[3] = patientMail.getMessage();
